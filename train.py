...
def train_joint():
    ...
    total_iter, mc_epochs, joint_epochs = 0, 2, 10
    ...
    '''
    dataset = MCDataset(o_folder, transform=CropThreeFrames(432))
    ...
    '''
    mc_weight = 'weight\\mcnet_sep_1th_epoch.pth'
    mcnet.load_state_dict(torch.load(mc_weight))
    for epoch in range(joint_epochs):
       
        dataset = JointDataset(o_folder, c_folder, transform=CropFourFrames(192))
        dataloader = DataLoader(dataset, batch_size=16, shuffle=True, num_workers=0)
        for idx, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
            total_iter += 1
